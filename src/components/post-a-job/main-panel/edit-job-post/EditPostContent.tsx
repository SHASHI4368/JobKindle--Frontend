...
import { NewJobType } from "../new-job/types";
import BasicInfo from "../new-job/basic-info";
import JobDetails from "../new-job/job-details";
import Skills from "../new-job/skills";
import { EditJobPostDialogProps } from ".";
import { useDispatch, useSelector } from "react-redux";
import { ViewPostsProps } from "../view-posts";
import { setCreateJobPostsData } from "@/redux/features/createJobPostsSlice";

const EditJobPostMain = ({ open, setOpen }: EditJobPostDialogProps) => {
  ...
  const handleJobPostUpdate = async () => {
    ...
    try {
      ...
      if (!jobPostId) throw new Error("No job post ID found");
      const response = await updateJobPost(jwt, jobPostId, body);
      if (response.success) {
        toast.success("Job post updated successfully");
        await getCreatedPosts();
      } else {
        toast.error(response.message || "Error updating job post");
      }
    } catch (error) {
  ...
  const getJobPostData = async () => {
    ...
    try {
      ...
      if (!jobPostId) throw new Error("No job post ID found");
      setLoading(true);
      const response = await getJobPostById(jwt, jobPostId);
      if (response.success) {
        setJobData({
          ...
          skills: response.data.skills.map((skill: any) => skill.name) || [],
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        `}
          ...
          `}
            ...
            `}
              >
                Fill in the details below to edit the job listing.
              </span>
            </div>
        ...
        <div className="md:flex hidden flex-row items-center justify-between">
          ...
          <div className="flex flex-row gap-4 items-center">
            <SubmitDialog
              screenSize=""
              handleSubmit={handleJobPostUpdate}
              submitText="Update Job Post"
              submitLoadingText="Saving..."
              description="You want to update the job post"
...
export default EditJobPostMain;
